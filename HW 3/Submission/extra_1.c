...
//
void sort_list(int q, int id_token) {
    ...
    // Sort local lists
    // for (my_id = 0; my_id < num_threads; my_id++) {
        my_list_size = ptr[id_token+1]-ptr[id_token];
        qsort(&list[ptr[id_token]], my_list_size, sizeof(int), compare_int);
        #pragma omp barrier
    // }
if (DEBUG) print_list(list, list_size); 
	...
	// for (my_id = 0; my_id < num_threads; my_id++) {
	    ...
	    my_own_blk = ((id_token >> level) << level);
	    my_own_idx = ptr[my_own_blk];

	    my_search_blk = ((id_token >> level) << level) ^ (1 << level);
	    my_search_idx = ptr[my_search_blk];
	    my_search_idx_max = my_search_idx+my_blk_size;

	    my_write_blk = ((id_token >> (level+1)) << (level+1));
	    ...
	    if (my_search_blk > my_own_blk) {
               idx = binary_search_lt(list[ptr[id_token]], list, my_search_idx, my_search_idx_max); 
	    } else {
               idx = binary_search_le(list[ptr[id_token]], list, my_search_idx, my_search_idx_max); 
	    }
	    my_search_count = idx - my_search_idx;
	    i_write = my_write_idx + my_search_count + (ptr[id_token]-my_own_idx); 
	    work[i_write] = list[ptr[id_token]];

	    // Linear search for 2nd element onwards
	    for (i = ptr[id_token]+1; i < ptr[id_token+1]; i++) {
	        if (my_search_blk > my_own_blk) {
	...
	// for (my_id = 0; my_id < num_threads; my_id++) {
        #pragma omp barrier
	    for (i = ptr[id_token]; i < ptr[id_token+1]; i++) {
	        list[i] = work[i];
	    } 
	// }
    #pragma omp barrier
...
// Serial merge sort 
    ...
    #pragma omp parallel
    {
        int id_token = omp_get_thread_num();
        sort_list(q, id_token);
    }
