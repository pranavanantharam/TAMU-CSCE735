...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	if (nbr_k > my_id) {
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&list_size_gt, 1, MPI_INT, nbr_k % sub_hypercube_size, 0, sub_hypercube_comm);


	    // MPI-3: Receive number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k % sub_hypercube_size, 1, sub_hypercube_comm, &status);
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&(list[idx]), list_size_gt, MPI_INT, nbr_k % sub_hypercube_size, 0, sub_hypercube_comm);

	    // MPI-5: Receive neighbor's list of elements that are less than or equal to pivot

	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list[0], nbr_list_size, MPI_INT, nbr_k % sub_hypercube_size, 1, sub_hypercube_comm, &status);
	    ...
	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k % sub_hypercube_size, 0, sub_hypercube_comm, &status);


	    // MPI-7: Send number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
		MPI_Send(&list_size_leq, 1, MPI_INT, nbr_k % sub_hypercube_size, 1, sub_hypercube_comm);
	    ...
	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list[0], nbr_list_size, MPI_INT, nbr_k % sub_hypercube_size, 0, sub_hypercube_comm, &status);


	    // MPI-9: Send list[0 ... idx-1] to neighbor

	    // ***** Add MPI call here *****
		MPI_Send(&list[0], list_size_leq, MPI_INT, nbr_k % sub_hypercube_size, 1, sub_hypercube_comm);
